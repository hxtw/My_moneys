...
class Dubbing extends Controller
{
    ...
    */
    public function list_index()
    {
        ...
        //查询易企秀的列表数据
        $du_list = Db::name('dubbing')->field('du_id as id,du_title as title,du_isstate as isstate')->where($du_list_where)->select();
        //模板输出
   ...
   public function  add_viadd()
   {
    ...
    public function Dubbing_add()
    {
        ...
        if (empty($video)) return json( 'vid_no');
        //配音上传信息赋值
        $data = array(
//            'du_id' =>$request->post('id','','trim,htmlspecialchars,addslashes'),
    ...
    */
    public function edit_index()
    {
        ...
        //where参数，sql语句
        $du_where['du_id']=$request->get('id','','trim');
        $edit=Db::name('dubbing')->where($du_where)->field('du_id as id,du_title as title,du_video as video')->find();
